...
let CustomifyFontSelectFields = ( function( $, exports, wp ) {
        const
            ...
            valueHolderSelector = '.sm_font_values',
            fontFamilySelector = '.sm_font_family',
            fontWeightSelector = '.sm_font_weight',
            fontSubsetsSelector = '.sm_font_subsets',
        ...
        function updateWeightField( option, wraper ) {
            ...
            if ( sm_settings.settings[id].load_all_weights || typeof variants === "undefined" || Object.keys( variants ).length < 2 || font_weights.data('disabled') !== undefined ) {
         ...
         * It collects values and saves them (encoded) into the `.sm_font_values` input's value
         */
        ...
        const renderFonts = function() {
            $( '.sm_font_family' ).select2().trigger( 'change' )
        };
