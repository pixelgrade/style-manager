;(function ($, window, document, undefined) {
	...
	$(document).ready(function () {
		...
		$.each(sm_settings.settings, function (key, el) {
					...
					setting.bind(function (to) {
                        ...
                        if (typeof $values !== "undefined" ) {
                            ...
                            let vls = get_CSS_values(this.id, $values),
                                ...
                                field_style = $('#sm_font_output_for_' + el.html_safe_option_id);
		...
		api('live_css_edit', function (setting) {
			setting.bind(function (new_text) {
				$('#sm_css_editor_output').text(new_text);
			});
		});
		...
		const get_CSS_code = function (ID, values) {
			...
			let field = sm_settings.settings[ID];
			let output = '';
		...
		const get_field_unit = function (ID, field) {
			let unit = '';
			if (typeof sm_settings.settings[ID] === "undefined" || typeof sm_settings.settings[ID].fields[field] === "undefined") {
				return unit;
			}

			if (typeof sm_settings.settings[ID].fields[field].unit !== "undefined") {
				return sm_settings.settings[ID].fields[field].unit;
			} else if (typeof sm_settings.settings[ID].fields[field][3] !== "undefined") {
				// in case of an associative array
				return sm_settings.settings[ID].fields[field][3];
			}
		};
