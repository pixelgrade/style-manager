...
class StyleManager_Font_Selector extends StyleManager_Singleton_Registry {
	...
	function add_theme_fonts( $active_font_family, $val ) {
		...
		echo '<optgroup label="' . esc_html__( 'Theme Fonts', 'style-manager' ) . '">';
		foreach ( $this->theme_fonts as $font ) {
			if ( ! empty( $font ) ) {
				//display the select option's HTML
				StyleManager_Customize_Font_Control::output_font_option( $font['family'], $active_font_family, $font, 'theme_font' );
			}
		}
		echo "</optgroup>";
	}
	...
	function maybe_decode_value( $value ) {
		...
		$to_return = sm_decodeURIComponent( $value );
		if ( is_string( $value ) ) {
	...
	function output_font_dynamic_style() {
		...
		self::$options_list = StyleManager_Customizer::getInstance()->get_all_options();

		StyleManager_Customizer::getInstance()->get_typography_fields( self::$options_list, 'type', 'font', self::$typo_settings );
		...
		foreach ( self::$typo_settings as $id => $font ) {
			if ( isset ( $font['value'] ) ) {
				...
				if ( ! is_array( $value ) || $value === null ) {
					$value = StyleManager_Customizer::getInstance()->get_font_defaults_value( str_replace( '"', '', $font['value'] ) );
				}
				...
				if ( ! sm_is_assoc( $value ) ) {
					$value = StyleManager_Customizer::getInstance()->standardize_non_associative_font_default( $value );
				}
...
//					$value['variants'] = $this->theme_fonts[ $value['font_family'] ]['variants'];
		...
		if ( ( ! empty ( $args['local_families'] ) || ! empty ( $args['google_families'] ) ) && sm_get_setting( 'enable_typography', true ) && sm_get_setting( 'typography_use_google_fonts', true ) ) {
		...
		foreach ( self::$typo_settings as $key => $font ) {
			...
			if ( $value === null ) {
				$value = StyleManager_Customizer::getInstance()->get_font_defaults_value( $font['value'] );
			}
