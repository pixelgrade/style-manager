...
class DesignAssets extends AbstractHookProvider {
	...
	const CACHE_KEY = 'style_manager_design_assets';
	const CACHE_TIMESTAMP_KEY = 'style_manager_design_assets_timestamp';
	...
	public function register_hooks() {
		...
		$this->add_action( 'style_manager/invalidate_all_caches', 'invalidate_cache', 1 );
	}
	...
	public function get( $skip_cache = false ): array {
		...
		$this->design_assets = apply_filters( 'style_manager/get_design_assets', $this->design_assets );
	...
	protected function maybe_fetch( $skip_cache = false ) {
		...
		if ( ! wp_doing_ajax() && defined('STYLE_MANAGER_ALWAYS_FETCH_DESIGN_ASSETS' ) && true === STYLE_MANAGER_ALWAYS_FETCH_DESIGN_ASSETS ) {
			$skip_cache = true;
		}
	...
	protected function maybe_load_theme_config_from_theme_root( array $design_assets ): array {
		...
		// We expect to get from the file include a $config variable with the entire Style Manager (partial) config.
		include $file;

		if ( ! isset( $config ) || ! is_array( $config ) || empty( $config['sections'] ) ) {
			// Alert the developers that things are not alright.
			_doing_it_wrong( __METHOD__, 'The Style Manager theme root config is not good - the `sections` entry is missing. Please check it! For now, we will ignore it.', null );
