...
class CloudFonts extends AbstractHookProvider {
	...
	public function register_hooks() {
		 ...
		 */
		add_filter( 'style_manager/get_cloud_fonts', [ $this, 'preprocess_config' ], 5, 1 );

		/*
		 * Add the cloud fonts to the Font Selector
		 */
		$this->add_filter( 'style_manager/cloud_fonts', 'add_cloud_fonts_to_font_selector', 10, 1 );

		/*
		 * Handle the cloud fonts preprocessing.
		 */
		add_filter( 'style_manager/get_cloud_system_fonts', [ $this, 'preprocess_config' ], 5, 1 );

		/*
		 * Add the cloud system fonts to the Font Selector
		 */
		$this->add_filter( 'style_manager/system_fonts', 'add_cloud_system_fonts_to_font_selector', 10, 1 );

		/*
		 * Add the cloud font categories to the list.
		 */
		$this->add_filter( 'style_manager/font_categories', 'add_cloud_font_categories', 10, 1 );
	}
	...
	public function get_cloud_fonts( $skip_cache = false ): array {
		...
		return apply_filters( 'style_manager/get_cloud_fonts', $config );
	}
	...
	public function get_system_fonts( $skip_cache = false ): array {
		...
		return apply_filters( 'style_manager/get_cloud_system_fonts', $config );
	}
	...
	public function get_categories( $skip_cache = false ): array {
		...
		return apply_filters( 'style_manager/get_cloud_font_categories', $categories );
	}
	...
	protected function get_default_system_fonts(): array {
		...
		return apply_filters( 'style_manager/default_system_fonts', $default_system_fonts );
	}
	...
	protected function get_default_cloud_fonts(): array {
		...
		return apply_filters( 'style_manager/default_cloud_fonts', $default_config );
	}
	...
	protected function get_default_categories(): array {
		...
		return apply_filters( 'style_manager/default_cloud_font_categories', $default_categories );
	}
	 ...
	 *
	 * Convert the cloud font list to a format suitable for Style Manager.
	 *
	 * @since 2.0.0
	 *
	 * @param array $config
