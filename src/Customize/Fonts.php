...
class Fonts extends AbstractHookProvider {
	...
	public function register_hooks() {
		/*
		 * Standardize the Customizer configuration for field types we can handle.
		 */
	...
	protected function init() {
		...
		if ( $this->plugin_settings->get( 'typography_system_fonts', 'yes' ) ) {
			$this->system_fonts = FontsHelper::standardizeFontsList( apply_filters( 'style_manager/system_fonts', [] ) );
		...
		add_filter( 'style_manager/localized_js_settings', [ $this, 'add_to_localized_data' ], 10, 1 );
	}
	 ...
	 * @return array
	 */
	public function standardize_global_config( array $config ): array {
	...
	public function standardize_font_fields_config( &$item, $key = false ) {
		...
		if ( isset( $item['type'] ) && 'font' === $item['type'] ) {
			...
			$subfieldsConfig = apply_filters( 'style_manager/default_font_subfields_config', [
				'font-family'     => true,
	...
	public function standardizeFontValue( $value, array $fontConfig = [] ): array {
		...
		return apply_filters( 'style_manager/standardized_font_value', $value, $fontConfig );
	}
	...
	public function output_cloud_fonts_select_options_group( $active_font_family, $current_value ) {
		// Allow others to add options here
		do_action( 'style_manager/font_family_before_cloud_fonts_options', $active_font_family, $current_value );
		...
		// Allow others to add options here
		do_action( 'style_manager/font_family_after_cloud_fonts_options', $active_font_family, $current_value );
	}

	public function output_theme_fonts_select_options_group( $active_font_family, $current_value ) {
		// Allow others to add options here
		do_action( 'style_manager/font_family_before_theme_fonts_options', $active_font_family, $current_value );
		...
		// Allow others to add options here
		do_action( 'style_manager/font_family_after_theme_fonts_options', $active_font_family, $current_value );
	}

	public function output_system_fonts_select_options_group( $active_font_family, $current_value ) {
		// Allow others to add options here
		do_action( 'style_manager/font_family_before_system_fonts_options', $active_font_family, $current_value );
		...
		// Allow others to add options here
		do_action( 'style_manager/font_family_after_system_fonts_options', $active_font_family, $current_value );
	}

	public function output_google_fonts_select_options_group( $active_font_family, $current_value ) {
		// Allow others to add options here
		do_action( 'style_manager/font_family_before_google_fonts_options', $active_font_family, $current_value );
		...
		// Allow others to add options here
		do_action( 'style_manager/font_family_after_google_fonts_options', $active_font_family, $current_value );
	}
	...
	protected function getFontStyle( array $fontConfig ): string {
		...
		$cssSelectors = apply_filters( 'style_manager/font_css_selector', FontsHelper::standardizeFontSelector( $fontConfig['selector'] ), $fontConfig );
	...
	protected function getCSSValue( array $value, array $fontConfig ): array {
		...
		if ( ! empty( $value['font_variant'] ) && ! FontsHelper::isFalsy( $value['font_variant'] ) ) {
			// Standardize it.
			$font_variant = \Pixelgrade\StyleManager\Utils\Fonts::standardizeFontVariant( $value['font_variant'] );
	...
	protected function maybe_load_google_fonts() {
		...
		if ( empty( $this->google_fonts ) ) {
			...
			if ( file_exists( $fonts_path ) ) {
				$this->google_fonts = apply_filters( 'style_manager/filter_google_fonts_list', require( $fonts_path ) );
